...
## Current contract and available evidence

The schema and review rules below are retained from the supplied archive, not independently certified as current submission requirements. Before producing a real submission, consult [openai-docs](openai-docs.md) and verify the current official import schema and review meanings. If these conflict with current official guidance, follow the current verified contract and explain the change. If current guidance cannot be reached, identify the output as a draft against the archive's v1 contract; do not call it ready for submission.

Use accessible source files, uploaded code, or connected repository tools. If the server or helper implementations are missing, request the relevant source rather than infer side effects from names. Reuse existing authorization to make requested code corrections; this reference does not impose a new approval when the user already authorized that exact work. Preparing the JSON does not itself submit or publish the app.

Contents: workflow; app metadata; annotation hints; output schemas; tool descriptors and CSP; test cases; JSON output contract; justifications; review reporting.

## Workflow

1. Inspect the MCP server codebase provided or made accessible for this task; use the project directory when available.
...
5. Compare tool annotations, tool names, tool descriptions, and CSP values against actual behavior. Correct them when source edits are already authorized; otherwise present the proposed changes and request authorization before editing source.
...
If tool names, descriptions, or CSP values are missing or inconsistent with behavior, use existing authorization for source corrections or ask before editing source when no such authorization exists. If approval does not arrive or the edit is ambiguous, keep the JSON truthful and report the finding.
...
`$schema` identifies the import file shape for editors and importers; the skill does not need to fetch it just to preserve the template. `tools` is required. `app_info`, `test_cases`, and `negative_test_cases` are optional in the schema, but generate them whenever the repo contains enough information. Do not include review-check findings in this JSON file.
