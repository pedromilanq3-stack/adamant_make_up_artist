...
Use an already connected OpenAI Docs MCP server at `https://developers.openai.com/mcp` first for current OpenAI developer guidance. This skill does not configure or install it. If unavailable, use the available browser or web tools with official OpenAI domains; if neither is available, state what cannot be verified. This skill also owns model selection, API model migration, and prompt-upgrade guidance.

## API Key Setup

For requests that require a live OpenAI API call, use [openai-platform-api-key](openai-platform-api-key.md) first when available. Missing credentials block only the live call. They do not block requested source or configuration edits, documentation retrieval, or offline, mocked, static, fixture-based, or syntax validation.
...
- **Explicitly named target:** preserve that model and fetch `https://developers.openai.com/api/docs/guides/latest-model?model=<requested-model>` through Docs MCP. URL-encode the actual requested model identifier. Do not substitute the current model or guidance for a different model.
...
1. Use tools exposed by an already connected `openaiDeveloperDocs` server, when available. Otherwise apply the same source-selection workflow through available web tools. Never invent MCP tool availability.
...
3. Run available, relevant repository checks for the changed path. With no executable repository, explain which checks remain unrun; do not claim execution from inspection alone.
