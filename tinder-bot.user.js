...
(() => {
  ...
  // Versões antigas gravavam apenas `true` e não podiam ser encerradas. O painel
  // antigo é removido para que salvar uma atualização não deixe uma UI órfã.
  document.getElementById("tinder-bot-panel")?.remove();
  ...
  const CONFIG = Object.freeze({
    selectors: {
      ...
      likeButtons: [
        ...
        'button[data-testid*="gamepadLike" i]',
        '[role="button"][data-testid*="gamepadLike" i]',
      ...
      rejectButtons: [
        ...
        'button[data-testid="gamepadNope"]',
        '[role="button"][data-testid="gamepadNope"]',
        'button[data-testid*="gamepadDislike" i]',
        '[role="button"][data-testid*="gamepadDislike" i]',
   ...
   */
  function inferProfileContainer() {
  ...
  function getCurrentProfile() {
    const candidates = CONFIG.selectors.profileCards
      ...
      .filter(visible);
    const hasLocalControl = (candidate) =>
      queryFirst(CONFIG.selectors.likeButtons, candidate) ||
  ...
  async function processProfile() {
    ...
    const profile = getCurrentProfile();
    if (!profile) {
      setStatus("AGUARDANDO perfil/botões");
      debug("Perfil não encontrado; aguardando mudança no DOM");
      return;
    }
    if (profile.fingerprint === state.lastHandledFingerprint) return;
  ...
  function mountUI() {
    const existing = document.getElementById("tinder-bot-panel");
    if (existing) {
    ...
    // O Tinder pode substituir o <body> inteiro durante a navegação SPA. Anexar ao
    // documentElement mantém o painel vivo durante essa recriação.
  ...
  function showPanel() {
    try {
      mountUI();
      const panel = document.getElementById("tinder-bot-panel");
      if (panel) {
        panel.hidden = false;
        // Regras globais do site não devem conseguir ocultar o controle de parada.
      ...
      console.error("[TinderBot] Não foi possível montar o painel:", error);
    }
  }
  ...
  function destroyInstance() {
    ...
    state.observer?.disconnect();
    document.getElementById("tinder-bot-panel")?.remove();
    removeEventListener("keydown", panelShortcut);
  }

  function panelShortcut(event) {
  ...
  state.routeTimer = setInterval(() => {
    ...
    if (!document.getElementById("tinder-bot-panel")) showPanel();
