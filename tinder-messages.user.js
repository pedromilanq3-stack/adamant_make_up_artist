// ==UserScript==
// @name         Tinder Web - Mensagens assistidas
// @namespace    local.tinder.messages.assistant
// @version      1.0.0
// @description  Prepara mensagens para conversas escolhidas pelo usuário; cada envio exige confirmação manual explícita.
...
(() => {
  ...
  if (window.__TINDER_MESSAGE_ASSISTANT__) return;
  window.__TINDER_MESSAGE_ASSISTANT__ = true;

  /**
   * Este script NUNCA envia nada sozinho. Ele só preenche o campo de mensagem de
   * conversas que o próprio usuário marcou e só clica em "Enviar" do Tinder depois de
   * um clique explícito em "CONFIRMAR ENVIO" dentro deste painel, por item. Não cria
   * contas, não resolve CAPTCHA, não tenta contornar limites de taxa da plataforma e
   * não guarda nada em disco: fila e log existem só em memória, até recarregar a
   * página. Todos os seletores dependentes da interface ficam em CONFIG.
   */
  const CONFIG = Object.freeze({
    selectors: {
      conversationItems: [
        'a[href^="/app/messages/"]',
        'a[href*="/app/messages/"]',
        '[data-testid="matchListItem"]',
        '[role="listitem"] a'
      ],
      conversationName: [
        '[data-testid="matchListItemName"]',
        'h3',
        'h4',
        'span'
      ],
    ...
    safetyText: [
      ...
      /limite (?:de mensagens|diário) atingido/i,
      /rate limit/i
    ],
  ...
  const debug = (...args) => ui.debug?.checked && console.debug("[TinderMessages]", ...args);
  ...
  function haltOnSafety(context) {
    ...
    state.stopped = true;
    state.current = null;
    writeLog("STOP", danger, null, `Automação interrompida (${context}). Fila local mantida, mas processamento parado.`);
  ...
  function conversationIdFromHref(href) {
    const match = href && href.match(/\/app\/messages\/([^/?#]+)/);
  ...
  async function prepareNext() {
    ...
    if (!input) {
      entry.status = "failed";
      writeLog("OPEN_FAIL", "Campo de mensagem não encontrado a tempo.", entry.name, "Item marcado como falho; não avança sozinho.");
      renderQueue();
  ...
  async function confirmSend() {
    ...
    if (!button || !visible(button) || !enabled(button)) {
      entry.status = "failed";
      writeLog("SEND_FAIL", "Botão de enviar não encontrado, invisível ou desabilitado.", entry.name, "Nenhum clique foi feito.");
      finishCurrent();
      return;
    }
    button.click();
  ...
  function renderConversationPicker(results) {
    ...
    if (!results.length) {
      ui.pickerList.textContent = "Nenhuma conversa visível encontrada. Role a lista do Tinder e busque de novo.";
      return;
    }
  ...
  function renderQueue() {
    ...
    for (const entry of state.queue) {
      ...
      row.innerHTML = `<b>${entry.name || "(sem nome)"}</b> <span class="tm-status">[${entry.status}]</span>`;
  ...
  function downloadLog(format) {
    ...
    link.download = `tinder-messages-log-${new Date().toISOString().replace(/[:.]/g, "-")}.${format}`;
    link.click();
    setTimeout(() => URL.revokeObjectURL(link.href), 1000);
  }

  function mountUI() {
    if (document.getElementById("tinder-messages-panel")) return;
    const panel = document.createElement("section");
    panel.id = "tinder-messages-panel";
    panel.innerHTML = `
      <style>
        #tinder-messages-panel{position:fixed;z-index:2147483647;left:16px;top:72px;width:340px;max-height:88vh;overflow:auto;padding:14px;border-radius:14px;background:#17171c;color:#fff;box-shadow:0 8px 30px #0008;font:13px/1.35 system-ui,sans-serif;border:1px solid #ffffff25}
        #tinder-messages-panel *{box-sizing:border-box}#tinder-messages-panel h2{font-size:15px;margin:0 0 8px;color:#4fd1c5}
        #tinder-messages-panel h3{font-size:12px;margin:12px 0 4px;color:#aaa;text-transform:uppercase}
        #tinder-messages-panel textarea{width:100%;min-height:60px;border:1px solid #ffffff35;border-radius:6px;padding:6px;background:#292930;color:#fff;font:inherit}
        #tinder-messages-panel input[type=number]{width:70px;border:1px solid #ffffff35;border-radius:6px;padding:4px;background:#292930;color:#fff}
        #tinder-messages-panel button{border:0;border-radius:7px;padding:6px 8px;cursor:pointer;font-weight:700;background:#3a3a44;color:#fff;margin:2px 3px 2px 0}
        #tinder-messages-panel button:disabled{opacity:.4;cursor:not-allowed}
        #tinder-messages-panel .tm-confirm{background:#21d07a}#tinder-messages-panel .tm-stop{background:#ff4458}
        #tinder-messages-panel .tm-status{opacity:.7}#tinder-messages-panel .tm-check{display:block;margin:3px 0}
        #tinder-messages-panel .tm-check input{margin-right:6px}#tinder-messages-panel .tm-picker{max-height:110px;overflow:auto;border:1px solid #ffffff20;border-radius:6px;padding:6px;margin-top:4px}
        #tinder-messages-panel .tm-queue-row{display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #ffffff15;padding:3px 0;font-size:12px}
        #tinder-messages-panel .tm-preview{white-space:pre-wrap;background:#ffffff10;border-radius:6px;padding:8px;margin:6px 0;font-size:12px}
        #tinder-messages-panel .tm-check-row{display:flex;gap:7px;align-items:center;margin:6px 0}
      </style>
      <h2>Mensagens assistidas <small>(local)</small></h2>
      ...
      <div class="tm-picker" data-ui="pickerList">Clique em "Buscar" para listar.</div>
      ...
      <div class="tm-check-row"><label class="tm-check"><input data-ui="dryRun" type="checkbox" checked> Dry Run (não clicar em enviar)</label></div>
      <div class="tm-check-row"><label>Limite da sessão<input data-ui="limit" type="number" min="1" max="${CONFIG.defaults.maxLimit}" value="${CONFIG.defaults.limit}"></label></div>
      <label class="tm-check"><input data-ui="debug" type="checkbox"> DEBUG no console</label>
      <div><button data-ui="prepareNext">Preparar próximo da fila</button></div>
      <div class="tm-preview" data-ui="preview">(nenhum item preparado)</div>
      <div>
        <button class="tm-confirm" data-ui="confirmSend" disabled>CONFIRMAR ENVIO</button>
        <button data-ui="skip" disabled>Pular</button>
        <button class="tm-stop" data-ui="stop">PARAR / limpar fila</button>
  ...
  // Reinstala o painel caso o Tinder recrie o body durante navegação SPA.
  setInterval(() => {
    if (!document.getElementById("tinder-messages-panel") && document.body) mountUI();
  }, 1000);
